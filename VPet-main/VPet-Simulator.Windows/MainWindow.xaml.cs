...
namespace VPet_Simulator.Windows
{
    ...
    public partial class MainWindow : WindowX
    {
        ...
        private ResearchLifeIpcServer researchLifeIpcServer;
        ...
        private void StartResearchLifeIpc()
        {
            if (!Args.ContainsLine("researchlife_ipc"))
                return;

            if (!int.TryParse(Args["researchlife_ipc"].Info, out var ipcPort) || ipcPort <= 0 || ipcPort > 65535)
                return;

            try
            {
                researchLifeIpcServer = new ResearchLifeIpcServer(this);
                researchLifeIpcServer.Start(ipcPort);
                Console.WriteLine($"Research Life IPC listening on 127.0.0.1:{ipcPort}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Research Life IPC failed: {ex.Message}");
                researchLifeIpcServer?.Dispose();
                researchLifeIpcServer = null;
            }
        }

        private void StopResearchLifeIpc()
        {
            researchLifeIpcServer?.Dispose();
            researchLifeIpcServer = null;
        }
