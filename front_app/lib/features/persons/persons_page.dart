...
class _PersonsPageState extends State<PersonsPage> {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        for (final event in entry.relatedItems) ...[
                          _RelatedItemTile(event: event, compact: false),
  ...
  Future<void> _showMergePersonDialog(_PersonGraphEntry entry) async {
    ...
    if (people.length < 2) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('至少需要两个人物才能执行合并。')));
      return;
    }
    ...
    return sessions.expand((session) {
      return session.people.map((person) {
        ...
        final relatedItems =
            session.events
                .where(
                  (event) => event.personNames.any(
                    (personName) => aliases.contains(personName),
                  ),
                )
                .toList()
...
class _PeopleWorkbench extends StatelessWidget {
  ...
  final void Function(String actionLabel, _PersonGraphEntry entry)
  onQuickAction;
  ...
  Map<String, Offset> _buildPositions(Size size, Offset center) {
    ...
    for (var index = 0; index < entries.length; index++) {
      ...
      final angle =
          ...
          ring * 0.22;
      final radius =
          availableRadius + ring * ringGap + (index.isOdd ? 10.0 : -8.0);
      final raw = Offset(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [tokens.sidebarSurface, tokens.accent],
        ),
...
class _AnchoredPersonPopover extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.tokens;
    final maxCardHeight = math
        .min(360, canvasSize.height - (_edgePadding * 2))
        .toDouble();
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (context) => const [
                    PopupMenuItem<String>(value: '编辑称谓', child: Text('编辑称谓')),
                    PopupMenuItem<String>(value: '合并人物', child: Text('合并人物')),
                  ],
                ),
                IconButton(
                  tooltip: '关闭',
            ...
            Container(
              ...
              decoration: BoxDecoration(
                border: Border(top: BorderSide(color: tokens.borderFaint)),
              ),
              child: Wrap(
                spacing: 10,
                runSpacing: 8,
...
class _RelatedItemTile extends StatelessWidget {
  const _RelatedItemTile({required this.event, required this.compact});
  ...
  Widget build(BuildContext context) {
    ...
    final iconColor = event.type == EventType.plan
        ? tokens.warmAccent
        : tokens.accent;
...
class _MergePeopleDialogState extends State<_MergePeopleDialog> {
      ...
      content: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            DropdownButtonFormField<String>(
              initialValue: _secondaryPersonId,
              decoration: const InputDecoration(
                labelText: '并入人物',
                border: OutlineInputBorder(),
              ),
  ...
  String _mergedAliasPreview(PersonProfile primary, PersonProfile secondary) {
    final aliases =
        <String>{...primary.aliases, ...secondary.aliases, secondary.name}
            .where((alias) => alias.trim().isNotEmpty && alias != primary.name)
            .toList();
...
class _PopoverLayout {
  ...
  static _PopoverLayout resolve({
    ...
    final left = desiredLeft
        .clamp(edgePadding, canvasSize.width - edgePadding - cardWidth)
        .toDouble();
    final top = (anchor.dy - cardHeight / 2)
        .clamp(edgePadding, canvasSize.height - edgePadding - cardHeight)
        .toDouble();
    final arrowTop = (anchor.dy - 7)
        .clamp(top + 18, top + cardHeight - 24)
        .toDouble();
