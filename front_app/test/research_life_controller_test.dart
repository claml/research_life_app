...
void main() {
  ...
  group('ResearchLifeController', () {
    test('returns a clear message for empty analysis input', () async {
      ...
      final message = await controller.analyzeCurrentInput();
    ...
    test(
      ...
      () async {
        ...
        await analysisController.analyzeCurrentInput();
...
''',
    ...
    test('loads confirmed sessions from sqlite on startup', () async {
      ...
      await firstController.analyzeCurrentInput();
...
''';
    ...
    test('toggles todo done and persists across restart', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_todo_toggle_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      ...
      final firstController = ResearchLifeController(
        ...
        localWorkspaceService: workspaceService,
        manualEventsRepository: manualEventsRepository,
        todoStatusRepository: todoRepository,
      );
      addTearDown(firstController.dispose);
      ...
      addTearDown(secondController.dispose);
      await secondController.ensureManualEventsLoaded();
      await secondController.ensureTodoStatusLoaded();

      expect(secondController.todoEvents, hasLength(1));
      expect(secondController.todoEvents.single.isDone, isTrue);
      expect(secondController.todoEvents.single.priority, TodoPriority.none);
    });

    test('sets todo priority and persists across restart', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_todo_priority_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      ...
      await firstController.setTodoPriority(
        firstController.todoEvents.single.id,
        TodoPriority.high,
      );
      await firstController.waitForPendingTodoStatusPersistence();
      expect(firstController.todoEvents.single.priority, TodoPriority.high);
      ...
      expect(secondController.todoEvents.single.priority, TodoPriority.high);
    });

    test('deleting a session cleans up its todo status', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_todo_delete_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      ...
      controller.inputController.text = '这周完成论文整理，下周准备实验。';
      await controller.analyzeCurrentInput();
      controller.confirmDraft();

      final planEvents = controller.todoEvents;
      expect(planEvents, isNotEmpty);
      await controller.toggleTodoDone(planEvents.first.id);
      await controller.waitForPendingTodoStatusPersistence();
      expect(await todoRepository.loadAll(), isNotEmpty);

      final sessionId = controller.sessionHistory.single.id;
      controller.deleteSessionRecord(sessionId);
      await controller.waitForPendingTodoStatusPersistence();

      expect(await todoRepository.loadAll(), isEmpty);
      expect(controller.todoEvents, isEmpty);
    });

    test('shows home todo hint only when pet is off and todos exist', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_home_todo_hint_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      ...
      expect(controller.showHomeTodoHint, isFalse);
      ...
      expect(controller.todayTodoEvents, isNotEmpty);
      expect(controller.showHomeTodoHint, isTrue);

      controller.dismissHomeTodoHint();
      expect(controller.showHomeTodoHint, isFalse);
    });

    test('persists glass settings through preferences', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_glass_settings_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      ...
      expect(secondController.glassSettings.blurSigma, 20);
      expect(secondController.glassSettings.opacity, 0.6);
      expect(secondController.glassSettings.noiseEnabled, isFalse);
    ...
    test('pin lock enables, verifies and persists across restart', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_pin_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      ...
      // 格式校验
      final invalid = await firstController.enablePinLock('1234');
      expect(invalid, contains('6 位数字'));

      // 重启后：加载 PIN 并进入锁定状态。
      final secondController = createController();
      expect(secondController.pinLockEnabled, isFalse);
      await secondController.ensurePinLockLoaded();
      expect(secondController.pinLockEnabled, isTrue);
      expect(secondController.isLocked, isTrue);

      // 解锁与关闭。
      expect(secondController.verifyPin('123456'), isTrue);
      expect(secondController.isLocked, isFalse);
      final disableMessage = await secondController.disablePinLock('123456');
      expect(disableMessage, contains('已关闭'));
      expect(secondController.pinLockEnabled, isFalse);

      final thirdController = createController();
      await thirdController.ensurePinLockLoaded();
      expect(thirdController.pinLockEnabled, isFalse);
    });

    test('lock and unlock toggle state', () async {
      final tempDir = await Directory.systemTemp.createTemp(
        'research_life_controller_pin_lock_test',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      final database = AppDatabase(NativeDatabase.memory());
      addTearDown(database.close);
      final controller = ResearchLifeController(
        ...
        preferencesRepository: PreferencesRepository(database),
      );
      addTearDown(controller.dispose);
      ...
      await controller.enablePinLock('567890');
      controller.lock();
