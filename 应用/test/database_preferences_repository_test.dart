...
void main() {
  ...
  test('saves and loads local LLM analysis settings', () async {
    ...
    await repository.saveLocalLlmAnalysisSettings(
      jsonEncode(settings.toJson()),
    );

    final stored = await repository.loadLocalLlmAnalysisSettings();
    final decoded = LocalLlmAnalysisSettings.fromJson(
      (jsonDecode(stored!) as Map).cast<String, Object?>(),
    );

    expect(decoded.enableLocalLlmAnalysis, isTrue);
    expect(decoded.ollamaBaseUrl, settings.ollamaBaseUrl);
    expect(decoded.ollamaModel, LocalLlmAnalysisSettings.defaultOllamaModel);
    expect(decoded.ollamaTimeoutSeconds, 60);
