...
class _ReadingPageState extends State<ReadingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final documents = controller.pdfDocuments;
  ...
  Future<void> _openFullscreenReader(
    ...
    await Navigator.of(context).push<void>(
      MaterialPageRoute(
        ...
        builder: (_) =>
            _FullscreenReadingRoute(document: document, controller: controller),
      ),
    );
  }
  ...
  Future<void> _confirmDeleteDocument(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('确定从阅读列表移除《${document.title}》吗？相关标注也会一起删除。'),
        actions: [
          TextButton(
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('阅读', style: Theme.of(context).textTheme.headlineMedium),
              const SizedBox(height: 8),
              Text(
                '管理本地 PDF 文献，阅读时选中文字即可添加高亮、下划线、删除线、波浪线或笔记。',
                style: Theme.of(
                  context,
...
class _PdfReaderState extends State<_PdfReader> {
  ...
  void initState() {
    ...
    _initialPageNumber = widget.document.lastPage < 1
        ? 1
  ...
  Widget _buildViewer(BuildContext context) {
    ...
    return PdfViewer.file(
      ...
      params: PdfViewerParams(
        ...
        matchTextColor: const Color(0x66FFE082),
        activeMatchTextColor: const Color(0x80FF8F00),
        ...
        children: [
          _buildSelectionToolbar(
            onHighlight: () => unawaited(
              _createAnnotation(params, PdfAnnotationKind.highlight),
            ),
            onUnderline: () => unawaited(
              _createAnnotation(params, PdfAnnotationKind.underline),
            ),
            onStrikethrough: () => unawaited(
              _createAnnotation(params, PdfAnnotationKind.strikethrough),
            ),
            onWavyUnderline: () => unawaited(
              _createAnnotation(params, PdfAnnotationKind.wavyUnderline),
            ),
  ...
  Future<void> _createAnnotationFromDelegate(
    ...
    if (kind == PdfAnnotationKind.note) {
      note = await _requestNoteText(selection.selectedText);
  ...
  Future<String?> _requestNoteText(String selectedText) async {
    if (!mounted) {
      return null;
    }
      ...
      for (final rect in annotation.rects) {
        ...
        switch (annotation.kind) {
          ...
          case PdfAnnotationKind.note:
            _paintNote(canvas, pageBounds, color);
  ...
  void _paintNote(Canvas canvas, Rect bounds, Color color) {
...
class _AnnotationListState extends State<_AnnotationList> {
  ...
  Future<void> _editNote(PdfTextAnnotation annotation) async {
    final textController = TextEditingController(text: annotation.note ?? '');
    final result = await showDialog<String>(
      context: context,
      builder: (dialogContext) => AlertDialog(
