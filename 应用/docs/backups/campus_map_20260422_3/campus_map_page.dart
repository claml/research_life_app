...
class _CampusMapPageState extends State<CampusMapPage> {
  ...
  bool _searchDockCollapsed = false;
      ...
      builder: (context, _) {
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            final searchResults = _searchDockCollapsed
                ? null
                : _buildSearchResults(
                    mapPlaces,
  ...
  Widget _buildSearchDock(
    ...
    final collapsed = _searchDockCollapsed;
    final zoneName = _publicMapScene?.zone.name ?? '未选择校区';

    return AnimatedContainer(
      duration: const Duration(milliseconds: 220),
      curve: Curves.easeOutCubic,
      width: collapsed ? 244 : 360,
      child: _GlassPanel(
        padding: EdgeInsets.fromLTRB(16, 16, 16, collapsed ? 16 : 14),
        child: AnimatedSize(
          duration: const Duration(milliseconds: 220),
          curve: Curves.easeOutCubic,
                ...
                children: [
                  ...
                  Material(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        setState(() {
                          _searchDockCollapsed = !_searchDockCollapsed;
                        });
                      },
                      icon: Icon(
                        collapsed
                  ...
                  children: [
                    if (_publicZones.isNotEmpty) ...[
                      ...
                      DropdownButtonFormField<int>(
                        key: ValueKey(
                          _publicMapScene?.zone.id ?? _publicZones.first.id,
                        ),
                        initialValue:
                            _publicMapScene?.zone.id ?? _publicZones.first.id,
                        items: [
  ...
  void _savePlace(
    ...
    if (editingPlace != null) {
      final success = controller.updateCampusPlace(
        ...
        zoneId: _publicMapScene?.zone.id,
        isMine: _editingMine,
      );
      if (!success) {
  ...
  List<CampusPlace> _placesForScene(
    ...
    return [
      for (final place in places)
        if (place.zoneId == scene.zone.id) place,
    ];
  }

  int _hiddenPlaceCountForScene(
    List<CampusPlace> places,
    PublicCampusMapScene? scene,
  ) {
    if (scene == null) {
...
class _CampusPlanPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (scene!.outlinePolygons.isNotEmpty) {
      final outlineFill = Paint()
        ..color = const Color(0xFFE3ECDD)
        ..style = PaintingStyle.fill;
      final outlineStroke = Paint()
        ..color = const Color(0xFF8AA17F)
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.4;

      for (final polygon in scene!.outlinePolygons) {
        final path = Path();
