...
Research Life 是一个本地优先的 Flutter Windows 桌面应用，用于管理研究生日常记录、周分析、日历事项、人物关系、PDF 阅读、校园地图地点、主页天气和 VPet 桌宠联动。
...
- 当前项目目录未检测到 Git 仓库信息，向外协作时建议压缩源码或先初始化版本管理
...
| 网络 | Dart `HttpClient` | 天气服务、VPet 本地 IPC |
...
- `D:\桌面\研究生活\VPet-main`：VPet 桌宠源码或构建产物，设置页可选择其 exe 作为伴随进程
- `D:\桌面\研究生活\桌宠形象`：桌宠图片素材
...
- `ResearchLifeController` 是主要应用服务和状态容器，负责分析、保存、导入、人物合并、日历事件、PDF 阅读记录、天气、VPet、主页图片等大部分流程。
- `features/*` 主要是 UI 页面，业务动作基本委托给 controller。
- `services/*` 放具体能力：规则分析、导入解析、校历解析、天气请求、本地工作区、VPet IPC、数据库仓储。
- 数据持久化通过 `services/database/repositories/*` 封装，底层 schema 在 `services/database/app_database.dart`。
- 应用关闭时会等待 pending persistence，按设置决定是否关闭 VPet 伴随进程。
...
| 设置 | `lib/features/settings/settings_page.dart` | 主题色、VPet 桌宠、数据目录、数据库备份、主页图片、校历固定格式导入、周报提示词模板 |
...
1. 设置页选择或自动发现 VPet 可执行文件。
2. `VPetCompanionService` 启动独立进程，并传入 `researchlife_ipc` 端口。
3. 应用通过 `http://127.0.0.1:18765` 调用 `/status`、`/say`、`/close`。
4. controller 可发送测试消息和未来任务提醒，退出时可按设置关闭 VPet。
...
- VPet 集成依赖本地 exe 和约定的 HTTP IPC 接口，目标 VPet 版本需要支持 `/status`、`/say`、`/close`。
...
- VPet 联动应该保持独立进程 IPC，还是嵌入窗口/插件化？
...
- `build/` 目录
- VPet 的完整构建产物，除非问题明确和桌宠集成有关

## 16. 可直接复制的咨询模板

```text
我有一个 Flutter Windows 桌面应用，叫 Research Life，用于本地管理研究生日常记录、周分析、日历事项、人物关系、PDF 阅读、校园地图和 VPet 桌宠联动。
...
- PDF 阅读用 pdfrx，天气用 Open-Meteo/ipapi，VPet 用本地 HTTP IPC
